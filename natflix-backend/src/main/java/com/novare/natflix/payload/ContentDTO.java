...
public class ContentDTO implements Comparable<ContentDTO> {
    ...
    public static Map<EType, List<ContentDTO>> build(List<Content> contents) throws NatflixException {
        ...
        for (Content content : contents) {
            ...
            switch (typeId) {
                ...
                case 3 -> {
                    List<ContentDTO> buildDocumentaries = buildDocumentaries(content.getDocumentaries(),
                            typeId);
    ...
    public static List<ContentDTO> buildDocumentaries(Set<Documentaries> documentaries,
            Integer typeId) {
    ...
    public static ContentDTO createResponse(
            AdditionalDetails additional, int id, int catId, int typeId) {
        ContentDTO response = new ContentDTO();
        response.setCatId(catId);
        response.setId(id++);
        response.setTypeId(typeId);
        response.setBannerUrl(additional.getBannerUrl());
        response.setLogUrl(additional.getLogoUrl());
        response.setPosterUrl(additional.getPosterUrl());
        response.setSummary(additional.getSummary());
        response.setThumbnailUrl(additional.getThumbnailUrl());
    ...
    private static AdditionalDetails createAdditionalModel(ContentDTO builder) {
        ...
        details.setBannerUrl(builder.getBannerUrl());
        details.setLogoUrl(builder.getLogUrl());
        details.setPosterUrl(builder.getPosterUrl());
        details.setSummary(builder.getSummary());
        details.setThumbnailUrl(builder.getThumbnailUrl());
